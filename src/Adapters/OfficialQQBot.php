...
class OfficialQQBot extends BaseAdapter
{
            ...
            case 0:
                switch ($this->事件类型) {
                    ...
                    case "GROUP_ADD_ROBOT":
                        ...
                        $this->用户ID = $解析["d"]["group_member_openid"] ?? $解析["d"]["user_openid"] ?? "";
                        break;
                    ...
                    case "GROUP_MESSAGE_CREATE":
                        ...
                        $this->消息原始数据 = $解析["d"];
                        $this->群原始ID = $解析["d"]["group_openid"] ?? "";
                        break;

                    case "GROUP_MEMBER_ADD":
                        $this->事件ID   = $解析["id"] ?? '';
                        $this->来源ID   = $解析["d"]["group_openid"] ?? '';
                        $this->用户ID   = $解析["d"]["group_member_openid"] ?? $解析["d"]["op_member_openid"] ?? '';
                        $this->用户信息 = "join";
                        $this->logger->info("[群成员加入] 群:{$this->来源ID} 用户:{$this->用户ID}");
                        break;
                    ...
                    case "GROUP_MEMBER_REMOVE":
                        ...
                        $this->用户ID   = $解析["d"]["group_member_openid"] ?? $解析["d"]["op_member_openid"] ?? '';
                        $this->用户信息 = "leave";
                        $this->logger->info("[群成员退出] 群:{$this->来源ID} 用户:{$this->用户ID}");
                        break;
    ...
    public function 发送(string $类型, mixed $主内容 = null, mixed $附加1 = null, mixed $附加2 = null): ?string
    {
        ...
        $url = match($this->事件类型) {
            ...
            "GROUP_AT_MESSAGE_CREATE", "GROUP_ADD_ROBOT", "GROUP_DEL_ROBOT", "GROUP_MESSAGE_CREATE", "GROUP_MEMBER_ADD", "GROUP_MEMBER_REMOVE"
